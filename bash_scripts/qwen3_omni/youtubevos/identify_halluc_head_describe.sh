...
"$PY" "$ROOT_DIR/method/qwen3_omni/run_attribution_parallel.py" \
    --input_file "$ROOT_DIR/results/qwen3_omni/YouTubeVOS_describe/sampled_entities.json" \
    --video_folder "$ROOT_DIR/data/YouTubeVOS/videos" \
    --model_path "$MODEL_PATH" --modal_type v \
    --output_path "$ROOT_DIR/results/qwen3_omni/YouTubeVOS_describe/attribution" \
