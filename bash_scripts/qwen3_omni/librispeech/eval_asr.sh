...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3} "$PY" "$ROOT_DIR/method/qwen3_omni/librispeech_attribution_eval.py" \
    --model_path "$MODEL_PATH" --split "${SPLIT:-test-other}" \
    --target_n "${N:-300}" --scan_limit "${SCAN_LIMIT:-2000}"
