...
"$PY" "$ROOT_DIR/method/qwen3_omni/run_attribution_parallel.py" \
    --input_file "$ROOT_DIR/results/qwen3_omni/LibriSpeech_test-other/sampled_entities.json" \
    --video_folder "$ROOT_DIR/data/LibriSpeech/test-other" \
    --model_path "$MODEL_PATH" --modal_type a \
    --output_path "$ROOT_DIR/results/qwen3_omni/LibriSpeech_test-other/attribution" \
    --gpu_groups "$GPU_GROUPS" --max_new_tokens 512
