...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3} "$PY" "$ROOT_DIR/method/qwen3_omni/eval.py" \
    ...
    --output_file "$ROOT_DIR/results/qwen3_omni/ActivityNet_describe/sampled_entities.json" \
    --QA_FILE "$ROOT_DIR/data/ActivityNet/QA.json" \
    --audioset_labels_file "$ROOT_DIR/data/ActivityNet/activitynet_labels.txt" \
    --n_per_category "${N:-200}" --tasks "ActivityNet Captioning"
