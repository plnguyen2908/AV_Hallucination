...
from .encoder import build_audio_tower, build_vision_tower
from .projector import build_audio_projector, build_vision_projector, load_mm_projector
...
class Videollama2MetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "mm_audio_tower"):
            self.audio_tower, audio_tower_cfg = build_audio_tower(
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if pretrain_mm_mlp_adapter is not None:
            if os.path.exists(pretrain_mm_mlp_adapter):
                ...
                is_local = False
                pretrain_mm_mlp_adapter = pretrain_mm_mlp_adapter.replace(
                    "mm_projector.bin", ""
                )
                pretrain_mm_mlp_adapter = (
                    pretrain_mm_mlp_adapter.strip("/").strip("\\").strip()
                )
                mm_projector_weights = load_mm_projector(pretrain_mm_mlp_adapter)
    ...
    def initialize_audio_modules(self, model_args, fsdp=None):
        ...
        self.config.use_mm_proj = True
        self.config.mm_projector_a_type = getattr(
            model_args, "mm_projector_a_type", "linear"
        )
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            self.mm_projector_a.load_state_dict(
                get_w(mm_projector_weights, "mm_projector_a"), strict=True
            )
...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images):
        num_frames = (
            self.config.num_frames if hasattr(self.config, "num_frames") else NUM_FRAMES
        )
    ...
    def temporal_aggregator(self, frames_features):
        ...
        if (
            ...
            or self.config.mm_projector_type == "linear"
        ):
        ...
        # *********** time  ************
            ...
            "tc_connector" in self.config.mm_projector_type
            or "tp_connector" in self.config.mm_projector_type
        ):
            video_features = self.get_model().mm_projector(frames_features)
        else:
            raise Exception(
                f"Unsupported projector type {self.config.mm_projector_type}!!!"
            )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (
            (vision_tower is None and audio_tower is None)
            or images is None
        ...
        if audio_tower is None:
            ...
            audio_tower is not None
            and vision_tower is not None
            and any(modal == "video" for (_, modal) in images)
        ):
            # [tensor, "image"]
            # [tensor, "audio"]
            # [tensor, "video"]
            ...
            if len(X_audio) > 0:
                ...
                audio_padding_mask = torch.zeros(
                    Xa_features.shape, device=self.device
                ).bool()
                audio_embedding, T, F = (
                    self.get_model()
                    .get_audio_tower()
            ...
            audio_padding_mask = torch.zeros(
                X_features.shape, device=self.device
            ).bool()
            audio_embedding, T, F = (
                self.get_model()
                .get_audio_tower()
                .extract_features(
                    X_features, padding_mask=audio_padding_mask, feature_only=True
                )
            )
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_multimodals = sum(
                (cur_input_ids == mm_token_idx).sum()
                for mm_token_idx in MODAL_INDEX_MAP.values()
            )
            # pure text input
            if num_multimodals == 0:
                ...
                cur_input_embeds = torch.cat(
                    [cur_input_embeds_1, cur_mm_features[0:0], cur_input_embeds_2],
                    dim=0,
                )
            ...
            mm_token_indices = torch.where(
                sum(
                    [
                        cur_input_ids == mm_token_idx
                        for mm_token_idx in MODAL_INDEX_MAP.values()
                    ]
                )
            )[0]
            while mm_token_indices.numel() > 0:
                ...
                cur_new_input_embeds.append(
                    self.get_model().embed_tokens(cur_input_ids[:mm_token_start])
                )
