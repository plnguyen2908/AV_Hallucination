"""run_asd_avhbench.py — AVCD baseline on AVHBench (yes/no) for Qwen3-Omni.
...
sys.path.insert(0, str(_HERE)); import asd as A  # noqa
...
def main(a):
    ...
    print(f"ASD AVHBench {a.split}: {len(rows)} Q  cd_alpha={a.cd_alpha}", flush=True)
    ...
    for i, (_, r) in enumerate(rows.iterrows()):
        ...
        try:
            logits, info = A.asd_answer_logits(model, inputs, use_aiv)
        except Exception as e:
    ...
    N = sum(tot.values()); C = sum(cor.values())
    print(f"\n=== ASD {a.split} n={N}: {C/N:.4f}  (gated {gated}) ===")
    for t in sorted(tot):
