"""Official WorldSense scoring, copied verbatim from VLMEvalKit
(vlmeval/dataset/utils/worldsense.py). Only imports are adapted so it runs
standalone: `get_dimension_rating` takes a DataFrame directly instead of
`load(path)`, and the LLM-judge `extract_option` fallback is omitted — we score
with the official `extract_characters_regex` exact-match (VLMEvalKit default
