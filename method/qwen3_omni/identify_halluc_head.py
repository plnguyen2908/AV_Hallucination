        ...
        with torch.inference_mode():
            outputs = model.generate(
                **inputs,
                ...
                return_audio=False,
                do_sample=False,
                # Composite ignores plain max_new_tokens (see utils.omni_infer).
                thinker_max_new_tokens=max_new_tokens,
                use_cache=True,
                pad_token_id=tokenizer.eos_token_id,
            )

            # Qwen3-Omni's composite generate returns (thinker_result, None)
            # even with return_audio=False. Under the monkeypatch,
            # thinker_result IS the (output, hal, non_hal) 3-tuple, so unwrap
            # the outer 2-tuple first.
            if (
                isinstance(outputs, tuple)
                and len(outputs) == 2
                and isinstance(outputs[0], tuple)
            ):
                outputs = outputs[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default=str(_REPO / "results/qwen3_omni/AudioSet/sampled_entities.json"),
    )
    parser.add_argument(
        "--video_folder",
        type=str,
    ...
    parser.add_argument("--model_path", type=str, default="/nobackup2/zyu362/hf_cache/hub/models--Qwen--Qwen3-Omni-30B-A3B-Instruct/snapshots/26291f793822fb6be9555850f06dfe95f2d7e695")
    parser.add_argument("--device_map", type=str, default="balanced_low_0",
                        help="HF device_map. balanced_low_0 works for an "
                             "8-GPU single replica; use 'balanced' for a "
                             "4-GPU replica (no GPU0 reservation).")
    parser.add_argument("--modal_type", type=str, default="a", choices=["a", "v", "av"])
    parser.add_argument(
        "--output_path",
        type=str,
        default=str(_REPO / "results/qwen3_omni/AudioSet/attribution"),
    )
    ...
    parser.add_argument("--no_score", action="store_true",
                        help="Run per-sample ablation only; skip the whole-set "
                             "contrastive_score (used for data-parallel shards, "
                             "where scoring runs once on the merged pth dir).")
