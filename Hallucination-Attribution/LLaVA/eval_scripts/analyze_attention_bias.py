...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(
        ...
        mscoco_generated_words_first_token = [
            tokenizer.decode([tokenizer(generated_word)["input_ids"][1]])
            for generated_word in mscoco_generated_words
        ]
        ...
        for idx, text_token in enumerate(text_tokens):
            if (
                text_token in mscoco_generated_words_first_token
            ):  # if the text token is an object in the generated caption
                hal_img_attn = 0.0
                hal_txt_attn = 0.0
                for layer_idx, head_idx in hal_attention_heads:
                    attention_scores = output_attentions[layer_idx][0][
                        0, head_idx, :, :
                    ].clone()
                    img_attention_score = attention_scores[
                        idx + image_start_idx + image_length - 1,
                        image_start_idx : image_start_idx + image_length,
                    ]
                    txt_attention_score = attention_scores[
                        idx + image_start_idx + image_length - 1,
                        image_start_idx + image_length : image_start_idx
                        + image_length
                        + idx,
                    ]
                    hal_img_attn += torch.sum(img_attention_score) / len(
                        hal_attention_heads
                    )
                    hal_txt_attn += torch.sum(txt_attention_score) / len(
                        hal_attention_heads
                    )
                ...
                for layer_idx, head_idx in non_hal_attention_heads:
                    ...
                    nonhal_img_attn += torch.sum(img_attention_score) / len(
                        non_hal_attention_heads
                    )
                    nonhal_txt_attn += torch.sum(txt_attention_score) / len(
                        non_hal_attention_heads
                    )

                attention_statics.append(
                    [
                        hal_img_attn.item(),
                        hal_txt_attn.item(),
                        nonhal_img_attn.item(),
                        nonhal_txt_attn.item(),
                    ]
                )
    ...
    torch.save(
        attention_statics, os.path.join(args.output_path, "attention_statics.pth")
    )


def plot_result(args):

    attention_statics = torch.load(
    ...
    # Plot bars
    ax.bar(
        x - width / 2, text_attention, width, label="Text Attention", color="skyblue"
    )
    ax.bar(
        x + width / 2,
        image_attention,
        width,
        label="Image Attention",
